...
# Runtime : the entire script is executed in less than 1 minute for 7 VMs in 2 Protection domains
...
$prismserver = 'MyNTNX'
$prismusername = 'mtbackup'
$prismpassword = 'MTPassword01!'
$vcserver = 'vcenter.local'
$vcuser = 'vcenterUser'
$vcpassword = 'vcPassword01!'
...
$connectstatus = Connect-NTNXCluster -Server $prismserver -UserName $prismusername -Password (ConvertTo-SecureString $prismpassword -AsPlainText -force) -AcceptInvalidSSLCerts
...
sleep 60
# Would be better to include a small wait and a loop testing snapshot creation and registration in protection domain
...
sleep 60
# Would be better to include a small wait and a loop testing all VMs in snapshots are restored in vCenter/VMM/Prism
...
# might not be possible, so might have to schedule a standard NBU backup job